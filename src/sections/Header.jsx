  ...
  return (
    <>
      ...
      <nav className="w-full bg-gray-100 flex justify-between items-center gap-1 lg:px-16 px-6 py-5 sticky top-0 z-40">
        ...
        <div id="mobile-nav" className={`${isMenuOpen ? 'flex ' : 'hidden'} rounded-md w-fit h-fit bg-themePurple p-4 absolute top-[80px] right-0`} onClick={closeMenu}>
            <ul className="flex flex-col">
              {
                navItems.map(({link,path})=>(
                  <Link key={path} className="text-themePurple uppercase font-semibold cursor-pointer px-1 py-3 rounded-lg hover:bg-themeYellow hover:text-black w-full text-center" to={path} spy={true} offset={-100} smooth={true}>{link}</Link>
                ))
              }
            </ul>
        </div>
